...
The blocking (opt-in) disposition of `--check-rank`: runs the identical rank-conformance audit but **exits non-zero** when any agent exceeds its rank ceiling, instead of warn-only. It implies `--check-rank`. This is the intended CI gate for the agent-position privilege axis — turn it on once the per-agent override policy (`PER_AGENT_OVERRIDES`) is ratified, so a new over-grant fails the build rather than only printing a warning. The finding severity is unchanged; only the exit code differs. The shipped Claude team validates clean under strict. Both `--check-rank` modes also print a **W19** advisory naming any agent that declares no capability key (an implicit maximal grant), which does not affect the exit code.

### `--check-wiring`

P1-3 verification that an emitted workspace-confinement `sandbox` block was actually **merged into the live `.claude/settings.json`** — the emitted block is inert until the operator merges it, and nothing else confirms the boundary took effect. Read-only and **output-only** (it reports booleans and the expected write roots, never echoes values from the operator's live settings, which may hold secrets). Exits non-zero when a confinement was emitted but is not live (missing, disabled, an open `allowUnsandboxedCommands` escape hatch, or mismatched write roots); exits `0` when the boundary is merged and enabled, or when no confinement was requested.
...
Run `--update --merge` across **every** agent-infrastructure workspace under a parent directory (and its subfolders) in one command. Replaces ad-hoc batch scripts and encodes the fleet-update lessons learned from production fleet runs.
