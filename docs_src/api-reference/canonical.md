...
Expands the [Canonical Agent Interface (CAI)](../interoperability.md) dict produced by [`interop.export_to_cai`](interop.md) into a
...
2. `agents/<slug>.md` — YAML front matter (`slug`, `name`, `description`, `source_path`,
   `capabilities` with the [canonical tool-scope vocabulary](capability-map.md), `handoffs`, optional
...
- [`multi_sync`](multi-sync.md) — the pinned-sync orchestrator that consumes this canonical hub.
