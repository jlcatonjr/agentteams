...
Format migration utilities for converting existing agent teams between supported frameworks while preserving body prose. The CAI-based sibling path is [`interop`](interop.md); for a runtime compatibility layer that keeps the source canonical rather than migrating it, use [`bridge`](bridge.md). The [interoperability guide](../interoperability.md) frames when to reach for each, and the [CLI Reference](../cli-reference.md) documents the `--convert-from` surface.
