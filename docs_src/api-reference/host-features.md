...
The flag is [`--target-host-features TOKENS`](../cli-reference.md) (comma-separated). Tokens flow onto `manifest["host_features"]` and are consumed by feature-gated emitters such as `bridge_subagents`, `hooks_emit`, `schedule_emit`, and `instructions_split`.

## Token → emitter → output at a glance

Each validated token routes to one emitter, which writes under `.claude/`, `.codex/`, or `.goose/`.
The figure is the visual companion to the token table below. Generated deterministically from
`scripts/gen_api_cluster_figures.py`.

![Host-feature token → emitter → output map](../book/figures/api-clusters/api-cluster-host-features.svg)
...
| `bridge:copilot-vscode-to-claude:parallelize` | Emit `.claude/skills/parallelize-plan/SKILL.md` — the Claude-only deterministic affordance for deriving fail-safe parallel waves from a plan-steps CSV's `depends_on` (see [`parallel_plan`](parallel-plan.md)). |
| `claude:sandbox` | Inject a `sandbox` block into the emitted `.claude/settings.hooks.example.json` (via the Claude adapter's `extra_output_files`). The block turns on Claude Code's native OS-level filesystem sandbox (macOS Seatbelt / Linux + WSL2 bubblewrap), confining every Bash command and child-process file write to `workspace_write_roots` (default `["."]` — the project root) with `allowUnsandboxedCommands: false`. Like the hooks block, it is **inert until the operator merges it** into their own `.claude/settings.json`; agentteams never writes `settings.json` directly. `.claude/` is auto-protected even inside the writable root. Usually selected via the `privilege_profile: confined`/`exclusive` project-description field, which expands to this token. See [Workspace Privilege Scoping](workspace-privilege-scoping.md). |
| `claude:mcp` | Emit `.claude/mcp-servers.agentteams.json` — an inert list of MCP server definitions (deliberately **not** `.mcp.json`; credentialed activation is withheld). Only first-party read-only servers scoped to an agent are auto-wired (see [`mcp_emit`](mcp-emit.md)). |
| `bridge:copilot-vscode-to-claude:mcp` | Same `.claude/mcp-servers.agentteams.json` emission as `claude:mcp`, via the canonical bridge token (see [`mcp_emit`](mcp-emit.md)). |
...
| `bridge:<src>-to-goose:subagents` | Emit one thin Goose subagent-stub recipe per source agent into `.goose/recipes/` when bridging a source team to Goose (reserved/bridge-owned slugs skipped; existing recipes never overwritten — see [`bridge_subagents_goose`](bridge-subagents-goose.md)). |
| `bridge:<src>-to-goose:mcp` | Wire selected MCP servers (read from the source team's inert `.claude/mcp-servers.agentteams.json`) as extensions into the emitted Goose bridge recipe (see [`mcp_emit`](mcp-emit.md)). |

> **Namespace scope:** this table lists the tokens — spanning the `claude`, `bridge:copilot-vscode-to-claude:*`, `bridge:<src>-to-goose:*`, `goose`, and `codex` namespaces — that have a wired-up effect today. `validate` / `parse_tokens` enforce a fixed allow-list — `_VALID_NAMESPACES` and a per-namespace `_KNOWN_FEATURES` set — not free-form syntactic well-formedness; a namespace or feature absent from those lists raises `HostFeatureError` however well-formed the token otherwise looks (e.g. `claude:not-a-real-feature` raises). Tokens whose namespace and feature are *both* allow-listed but not in this table — e.g. bare `claude:hooks` or `claude:subagents` — pass validation and simply produce no emission unless an emitter is looking for them.
...
- [`bridge`](bridge.md) — the host-agnostic bridge that consumes `manifest["host_features"]`.
- [`mcp_detect`](mcp-detect.md) — the analyze-time MCP-suitability rubric (distinct from the `*:mcp` emission tokens above).
