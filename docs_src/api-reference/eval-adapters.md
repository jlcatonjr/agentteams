...
These adapters translate the framework-neutral suite from [`agentteams.eval_suite`](eval-suite.md) into framework-specific runnable artifacts. Adapter modules are code generators and intentionally avoid runtime dependencies on target eval frameworks.
...
- [`behavioral_drift`](behavioral-drift.md) — verifies a recorded execution trajectory against the neutral suite.
- [`model_routing`](model-routing.md) — the sibling cost/capability contract emitted from the same manifest.
