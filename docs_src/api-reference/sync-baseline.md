...
Real-content baseline snapshot writer for native↔canonical agent synchronization.

> Source: `agentteams/sync_baseline.py`

Follows the `front_matter_baseline` precedent from `build-log.json`: store **actual content**
(not a hash) at the moment canonical is materialized into or absorbed from a given framework,
so a later run can classify *why* something differs rather than just detect *that* it does.
