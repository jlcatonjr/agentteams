...
Recent behavior includes conservative [retrieval-integration](../interoperability.md) inference. If a description omits `retrieval_integration`, directory supplementation may infer mode, query/maintenance entrypoints, trigger sources, source-of-truth hints, staleness SLO, and trigger-contract version from repository files.

> *Source: `agentteams/ingest.py`*

## Pipeline at a glance

`ingest` is stage one of the generate pipeline. The figure traces the whole flow — the brief
through `ingest` → [`analyze`](analyze.md) → [`render`](render.md) → framework adapter →
[`emit`](emit.md), with [`manifest_format`](manifest-format.md) and [`fences`](fences.md) as
carve-outs of `analyze` and `emit`. Generated deterministically from `scripts/gen_api_cluster_figures.py`.

![Pipeline data-flow: ingest → analyze → render → emit](../book/figures/api-clusters/api-cluster-pipeline.svg)
...
Inference output is normalized to the retrieval contract shape consumed by [`analyze.build_manifest()`](analyze.md).
