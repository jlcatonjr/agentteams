...
| [`analyze_tools`](analyze-tools.md) | Tool importance classification + tool/reference-doc detection (carved from analyze) |
...
| [`cli`](cli.md) | CLI package decomposition — which module owns which stage (flags live in [the CLI Reference](../cli-reference.md)) |
...
This index does not cover every module. **22 top-level public modules have no API
...
not removed from the baseline. It has ratcheted 22 → 20 → 22 — the drop as pages landed,
the return as the `interop_helpers` and `provenance` carve-outs were baselined with a
recorded reason (a documented deferral, never a silent one — `provenance` is a small
stdlib stamp helper documented at each consumer's sidecar, not yet warranting its own page).
