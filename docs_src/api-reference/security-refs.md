...
- `SECURITY_CURRENT_THREATS_SUMMARY`: Formatted KEV/CVE threat summary with EPSS and (optional) CVSS scores.
- `SECURITY_PREVENTION_PLAYBOOK`: Markdown prevention/remediation playbook.
...
- [Supply-chain integrity controls](../security-hardening-guide.md) validate all responses against an explicit exact-match hostname allowlist.
...
3. **Stale data handling** — see the stale-cache degradation described under **Behavior Notes** above.
...
- [`framework_research`](framework-research.md) — the framework-drift research stage that mirrors this module's build-placeholders contract.
- [`redteam`](redteam.md) — the standing red-team battery that correlates this threat-intel cache against the repo.
- [`scan`](scan.md) — the proactive security scanner over generated agent files.
