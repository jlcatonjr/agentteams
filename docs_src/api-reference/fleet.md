...
- [`update_report`](update-report.md) — the durable per-run record each in-process `--update` target leaves.
- [`drift`](drift.md) — the template-to-instance drift detection the per-target updates rely on.
- [Cross-Repository Coordination Guide](../cross-repository-coordination-guide.md) — coordinating agent-infrastructure changes across many workspaces.
- [Update Lifecycle Guide](../update-lifecycle-guide.md) — where a fleet run sits in the broader update lifecycle.
