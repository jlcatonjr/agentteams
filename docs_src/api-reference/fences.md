...
[Section-fencing](../section-fencing-guide.md) internals: the `AGENTTEAMS:BEGIN/END` regexes, `MergeResult`, fenced-region
...
Carved from [`emit.py`](emit.md) (CH-07 line ceiling) and re-exported there, so `drift`, [`fence_inject`](fence-inject.md),
