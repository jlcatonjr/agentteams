...
Manage external CSV log files for [cross-repository coordination](../cross-repository-coordination-guide.md) and security decisions.
...
Storing the logs as CSV under `references/` (rather than inline markdown tables) keeps them machine-readable, prevents agent-file bloat as logs accumulate, and stays version-control friendly.

Each row is idempotent (same timestamp, actor, and action key can be re-added without duplication issues).

## See also

- [PR management](pr-management.md) — the PR-lifecycle coordination surface these coordination and security-decision logs feed.
