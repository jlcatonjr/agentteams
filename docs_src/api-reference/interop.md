...
Canonical Agent Interface (CAI)-based interoperability pipeline across supported frameworks. For a body-preserving format migration between frameworks see [`convert`](convert.md); for a lightweight runtime compatibility layer that keeps the source canonical see [`bridge`](bridge.md); the durable CAI directory format is described in [`canonical`](canonical.md). The [interoperability guide](../interoperability.md) frames when to reach for each.
