...
Answers the question: *"Did the agent team run the workflow it was specified to run?"* by comparing an actual execution trajectory (from Phase 1 replay substrate) against the behavioral spec in the [eval-suite](eval-suite.md) (Phase 2). Orthogonal to template drift (see [`drift`](drift.md)).
...
4. **Typed-payload continuity** — Handoff payloads match their schema IDs (reuses `audit_handoff_chain` from [`handoff_payloads`](handoff_payloads.md))
...
- `severity`: The four `BEHAVIOR_*` codes are always `"HARD"` (behavioral divergence is critical). The reused `PAYLOAD_UNTYPED` code carries the dated WARN→HARD severity instead — `"WARN"` before the `2026-07-01` cutoff and `"HARD"` on or after it (see [`handoff_payloads`](handoff_payloads.md)`._payload_untyped_severity`). That cutoff has now passed, so `PAYLOAD_UNTYPED` findings emit at `"HARD"` today; the WARN branch is retained only for reproducing pre-cutoff runs.
...
- Reuses `audit_handoff_chain()` (from [`handoff_payloads`](handoff_payloads.md)) to check typed-payload continuity
...
The `behavioral_drift` module consumes the `handoff-chain` scenarios from the eval-suite; the framework-specific runnable form of the same suite is generated by [`eval_adapters`](eval-adapters.md). Each scenario specifies:
...
## Finding Reuse (shared with `handoff_payloads`)

Payload-schema continuity findings reuse the `Finding` dataclass and the `audit_handoff_chain()` function from [`handoff_payloads`](handoff_payloads.md). This provides a uniform finding shape across template drift, behavioral drift, and payload validation.
