...
Extracted from `analyze.py` under CH-07. `analyze` re-exports `_plan_output_files`, so
...
still ships in the wheel and still gets swept by drift detection — it just never
