...
Consumer-side template pinning — the trust root for security review §4.6. The consuming repository records the template digests it trusts in `<project>/.agentteams/template-pins.json` and **commits that file**; every later run compares the installed templates against it.
