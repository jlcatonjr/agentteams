...
consumer repositories through [`--update --merge`](../update-lifecycle-guide.md). Detects [drift](drift.md)
between published Claude Code / GitHub Copilot agent-infrastructure
specifications and this project's [framework-adapter constants](frameworks.md);
...
- `COPILOT_EXPERT_REF_REL` = `references/copilot-agent-infrastructure-expert.md`
  — the pre-split shared Copilot file, superseded by the per-surface
  `copilot_vscode`/`copilot_cli` targets but kept allow-listed so a patch
  proposal built from a stale, pre-split cached snapshot still resolves to
  an allow-listed path instead of being rejected.

Deriving from the registry keeps this set from going stale independently
of `FRAMEWORK_REGISTRY`: any change targeting a path outside the set is
still rejected, but the set updates automatically when a framework gains
its own `expert_ref`.
