...
Wraps a legacy file's body with canonical `AGENTTEAMS:BEGIN`/`END` [fence markers](fences.md) so it becomes eligible for merge-mode `--update` runs without requiring `--overwrite`. Used during cross-repository target remediation when legacy files lack fence metadata.
...
**Design note:** Matches the fence ID that [`emit._normalize_generated_content`](emit.md) uses for whole-body wraps, so a later `--update --merge` cleanly replaces in-place without duplication.
...
This is particularly useful for cross-repository target updates where legacy agents lack fence markers. See the [section-fencing guide](../section-fencing-guide.md) for how fenced regions are defined and the [update-lifecycle guide](../update-lifecycle-guide.md) for how merge-mode updates consume them.
