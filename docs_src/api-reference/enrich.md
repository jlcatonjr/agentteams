...
- In strict mode ([`--strict-manual-placeholders`](../cli-reference.md), default in `--self`), those placeholders remain unresolved and are handled by the enrichment/defaults-audit flow.
...
- [`emit`](emit.md) — produces the rendered `file_map` enrich scans and enriches, and writes the final `SETUP-REQUIRED.md` into the emitted team.
- [`feature_audit`](feature-audit.md) — verifies documented features still function; the scheduled audit companion to enrich's default-value audit.
