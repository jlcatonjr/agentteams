...
[`--fleet`](fleet.md), [`--bridge-merge`](bridge.md) and `--recipe-check` all leave a report file. `--update`,
the most common operation, left none: preserved fences, skipped legacy files and
retrofitted markers went to stdout and died with the scrollback. See the [update lifecycle guide](../update-lifecycle-guide.md) for where this report sits in a run.
...
The single entry point the [CLI](cli.md) calls. Wraps `write_report` and absorbs any write
