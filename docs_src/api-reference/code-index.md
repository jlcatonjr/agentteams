...
the documentation for those APIs (`api-doc`) — the two `api-*` kinds collected
statically by [`code_sources`](code-sources.md). Every indexed document carries a
...
See the [CLI reference](../cli-reference.md) for the complete code-index flag surface.
...
3. **Optional pre-commit warm-up** ([`--install-git-hooks --code-index-hook`](git-hooks.md),
