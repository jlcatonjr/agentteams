...
Lightweight cross-framework compatibility bridges that preserve source canonical documentation. Unlike [`convert`](convert.md), which migrates a team into another framework's format, the bridge keeps the source canonical and emits a thin compatibility layer beside it; the CAI transport path is [`interop`](interop.md). Bridge-merge refreshes fit into the broader [update lifecycle guide](../update-lifecycle-guide.md).
