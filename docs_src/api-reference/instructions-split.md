...
Cache-aware `CLAUDE.md` synthesis for the `copilot-vscode → claude` [bridge](bridge.md). Replaces the bridge's default pointer-only `CLAUDE.md` with a layout that maximises Claude's prompt-cache hit rate: a stable preamble (canonical `copilot-instructions.md` bytes verbatim) followed by a dynamic boundary marker and a small attribution stanza.
...
- [`hooks_emit`](hooks-emit.md) — sibling `copilot-vscode → claude` host-feature emitter.
- [`schedule_emit`](schedule-emit.md) — sibling `copilot-vscode → claude` host-feature emitter.
