...
Per-agent Claude subagent stub emitter for the `copilot-vscode → claude` [bridge](bridge.md). Stubs delegate to the canonical copilot-vscode source agent body via a `Read` directive and carry `source_sha256` provenance for drift detection. Source files are never modified.
...
- [`bridge_sources`](bridge-sources.md) — source-team inventory, hashing, and the freshness check feeding this emitter.
- [`bridge_subagents_goose`](bridge-subagents-goose.md) — the Goose-path parity emitter.
