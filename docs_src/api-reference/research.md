...
## Backend fallback at a glance

`search.web_search` (a thin wrapper over `web_search_verbose`) tries the backend chain in order
(unavailable ones skipped, failures degrade never raise). No hits **and not challenged** → an honest
empty list immediately ("nothing matched"). Only a challenge (a `202` interstitial) triggers
`_broadening_forms` to broaden the query **progressively** toward a floor and retry — and a still-
challenged empty is flagged as such, not as "nothing matched". Generated deterministically from
`scripts/gen_api_decision_figures.py` (Graphviz).

![research web_search backend fallback decision tree](../book/figures/api-clusters/api-decision-research-backends.svg)
...
**Honesty ceiling** (stated once here, back-referenced from each submodule below): allowlisted-domain
retrieval is provenance, not correctness — "reputable" is never "true." Claim-verification verdicts
are `"survived"` or `"refuted"` — never `"verified"` or `"proven"`.
...
**Honesty ceiling** (see top of page) — scholarly specifics: a hit is not a claim the work is
correct, replicated, relevant, or **un-retracted** — retraction status is not checked. Nothing is
inferred, normalised, or filled in when a source omits it, which is what makes the output safe to
build a bibliography from.
...
The **same honesty ceiling applies to all three** (see top of page) — worth flagging only because a
longer list reads as more authoritative than a short one. A consuming project is expected to edit
them, not inherit them uncritically. An official statistics series, for instance, is authoritative
about what it measured, which is not the same as being the right series for a question.
...
Dataclasses. `Verdict.status` is `Literal["survived", "refuted"]` (never `"verified"`/`"proven"` —
see the honesty ceiling at the top of this page).
...
Security-posture siblings:

- [Security Hardening Guide](../security-hardening-guide.md)
- [Security Workflow Glossary](../security-workflow-glossary.md)
- [`scan`](scan.md) — proactive security scan for generated agent files
- [`redteam`](redteam.md) — audit internals + corpus coverage/density
