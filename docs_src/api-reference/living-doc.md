...
Carved out of [`audit.py`](audit.md) to hold that module under the CH-07 line ceiling.

## Why the scope is *unfenced* prose only

A date inside an [`AGENTTEAMS:*` fence](fences.md) is module-generated and expected — build
...
- [`feature_audit`](feature-audit.md) — a sibling conformance check that likewise reports rather than gates.
