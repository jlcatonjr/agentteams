...
## Retrieval families at a glance

`memory_index` (prose → BM25 / sparse vector) is one of two retrieval families; the other is
[`code_index`](code-index.md) (repo scripts → [`code_sources`](code-sources.md) partitions), with
[`memory_index_incremental`](memory-index-incremental.md) as an in-place-patching satellite.
Generated deterministically from `scripts/gen_api_cluster_figures.py`.

![Retrieval topology: memory & code indexes](../book/figures/api-clusters/api-cluster-retrieval.svg)
...
- `root` (`Path | None`, keyword-only) — Project root. When given, each document's `path` is stored **relative to it**; the index is a committed artifact, so absolute paths leak the operator's home directory and username (2135 in this repository's own index before the change). Relativization happens here rather than at serialization so `source_fingerprint`, derived from the document paths, describes the paths actually stored. A source outside `root` stays absolute — it is genuinely external. Default: `None`, preserving absolute paths for existing callers.
...
**Algorithm:** Sparse tf·idf vector-space scoring with cosine similarity — stdlib only, no dense embeddings (see [Design Principles](#design-principles)).
...
Use `--refresh-index` after editing durable sources (for example `workSummaries/`, `CHANGELOG.md`, `README.md`, `docs_src/*.md`, `references/*.md`) before query-heavy sessions. See the [CLI reference](../cli-reference.md) for the full index-flag surface.
...
Incremental update mode (feature-flagged) — see [`memory_index_incremental`](memory-index-incremental.md):
...
- **Stale detection** (via `is_index_stale()`, see the [stale-detection guide](../stale-detection-guide.md)) — Query CLI auto-refreshes stale indexes; callers may still refresh proactively
...
Empirically: corpus expanded 69 → 198 docs on `example-collector`, lexical reliable rate 3/4 → 4/4, vector reliable 0/9 → 3/9 + 1 candidate.
...
- [`code_index`](code-index.md) — the parallel retrieval index over repository scripts and the external APIs they call.
