...
Generalized three-way classifier for native↔canonical agent [synchronization](multi-sync.md).

> *Source: `agentteams/sync_classifier.py`*

Generalizes [`front_matter_merge._merge_front_matter`](front-matter-reconcile.md)'s proven three-way decision table from
...
both-moved-conflict, with a **[capability-key carve-out](capability-map.md)** (§6.1) that always routes
...
- `NATIVE_MOVED` — native edited, canonical unchanged since [baseline](sync-baseline.md)
