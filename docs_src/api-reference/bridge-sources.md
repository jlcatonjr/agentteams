...
[bridge-freshness check](../stale-detection-guide.md) for the lightweight bridge. Framework-aware: markdown agent
...
Carved from [`bridge.py`](bridge.md) (CH-07 line ceiling) and re-exported there, so importers
...
Same construction as [`memory_index._documents_fingerprint`](memory-index.md), this repository's existing
precedent for a path/hash fingerprint.

## See also

- [`bridge_subagents`](bridge-subagents.md) — the claude-path subagent emitter that consumes these source agents.
