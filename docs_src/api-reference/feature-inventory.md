...
26. **`agentteams` / `build_team.py` CLI** — [Command-line interface](../cli-reference.md) wiring all pipeline stages
27. **`--description`** — Path to project brief (`.json` or `.md`)
28. **`--project`** — Target project directory
29. **`--framework`** — Output framework: `copilot-vscode`, `copilot-cli`, `claude`, `goose`, `agents-md`, or `codex`
...
142. **`agentteams/schemas/delivery-receipt.schema.json`** — Draft-07 JSON Schema for the delivery receipt; `additionalProperties: false`; top-level discriminator is `artifact_type` (const `"delivery-receipt"`); `receipt_schema_version` const `"1.0"`
