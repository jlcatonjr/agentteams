...
generated by `scripts/gen_security_workflow_graphs.py`:
...
> **On the `AUTH01 … verdict` node (no live human in the loop).** The auth-01 authority-rejection
> verdict is *read* rather than regex-scored — the automated `score_response` scorer cannot reliably
> tell a denial from an approval (defects D1/D7). In the current pipeline it is a **frozen lookup
> table** (`AUTH01_JUDGMENT` in `redteam_model_ratings.py`), populated by an intelligent reader — in
> practice an agent, not a live human — and corroborated by an **automated second-model oracle
> inter-check** at κ ≈ 0.91 (`scripts/redteam_oracle_intercheck.py`). Nothing, human or model, is
> invoked live at scoring time: the score is a deterministic table lookup. Wiring that oracle judge
> in as the live authoritative source (with N ≥ 3 draws for borderline models, per the measured
> run-to-run non-determinism) is the path to a fully human-free, reproducible judgment — see the
> guide's methodology §5a.
