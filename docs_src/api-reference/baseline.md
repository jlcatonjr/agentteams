...
Deterministic SHA-256 manifests of an [emission output tree](emit.md) for regression detection. Used by the CLI flags `--capture-baseline PATH` and `--check-baseline PATH`, and consumed by `tests/test_baseline.py` to pin per-target emission contracts (`tests/baselines/<team>-<framework>.json`).
...
- [`drift`](drift.md) — the template→artifact drift scan (a related but distinct regression signal).
- [Update lifecycle guide](../update-lifecycle-guide.md) — where baseline capture/check fits into the update flow.
