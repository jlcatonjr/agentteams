...
Opt-in subagent-stub-recipe emitter for the `… → goose` [bridge](bridge.md) — parity with
[`bridge_subagents`](bridge-subagents.md) (the claude path).

> Source: `agentteams/bridge_subagents_goose.py`

---

Gated by the `bridge:<source>-to-goose:subagents` [host-feature token](host-features.md) (default off, so
...
[`agentteams.frameworks.goose._emit_recipe`](frameworks.md)), skipping reserved/bridge-owned slugs
