...
**Recovery (W22 data-loss safety net):** The **default policy is `--shrink-policy=preserve`**: a fence whose new render is materially smaller than the existing body is **not** rewritten — the enriched body is kept, the shrink is suppressed, and a Notice is emitted. Nothing is lost and no sidecar is produced (non-shrinking fences in the same file still update normally). Under `--shrink-policy=warn` the smaller body **is** written, and as a safety net the full pre-merge body of every fence that triggered a shrink notice is persisted to a sidecar at `<output_dir>/.agentteams-backups/<ts>/<rel_path>.lost.<sid>.md`, with the Notice annotated `— recovery: <sidecar-path>`; to recover, copy the lost text from the sidecar into the USER-EDITABLE gap below the fence, then re-run. (`halt` aborts on a shrink; `allow` writes the smaller body silently with no sidecar.)
