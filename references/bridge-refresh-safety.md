...
**Origin:** 2026-05-27 incident — a Phase-2-through-Phase-4 development cycle used `--bridge-refresh` against `example-research` and `example-collector` test teams. `--bridge-refresh` is **explicitly destructive** at the target: it overwrites `CLAUDE.md`, `.claude/README.md`, `.claude/agent-team.md`, `.claude/quickstart-snippet.md`, and `.claude/skills/recall.md` unconditionally. Both test teams had user-authored content in those files (including `example-collector`'s `Retrieval-First Rule (MANDATORY)` and the user's full `sync_claude_agent_team.py`-generated agent inventory). The destructive overwrite silently replaced that content. Recovery was possible via `git checkout HEAD --` only because the files were tracked. The same operation against an untracked or recently-edited unstaged file would have been unrecoverable.
...
> **⚠ example-collector is BRIDGE-DEPRECATED as of 2026-08-14** (operator instruction;
> plan `tmp/by-week/2026-W33/example-collector-bridge-deprecation`). Its bridge layer
...
The two designated test teams (`example-research`, `example-collector`) are **not disposable**. They are real working repositories with user-authored documentation and active project history. Treat them like any other external consumer repository:
...
## Privilege scoping is NOT propagated by a bridge (explicit non-goal)

Workspace privilege scoping — the emitted Claude `sandbox` block (P1 write-confinement,
P3a read-exclusion), P2 capability grants, and the P3b inbound-hardening advisory — is a
property of **a specific team on a specific operator's host**, not portable metadata a
bridge should carry into a foreign repository. Bridging privilege state is therefore an
explicit **non-goal**, for concrete safety reasons:

- **A sandbox boundary is host- and workspace-specific.** `workspace_write_roots` and any
  `resolve_deny_read_abspath`-resolved `denyRead` paths name the *generating* machine's
...
**What this means in practice:** a `--bridge-refresh`/`--bridge-merge` neither emits nor
carries a target's `privilege_profile`, sandbox block, grants, or advisory. If a bridged
target needs confinement, set its own `privilege_profile` and (re)generate it **natively**
on the host that runs it — do not expect the source team's boundary to follow the bridge.
Re-verify enforcement on that host (see the workspace-privilege-scoping "Verifying
enforcement on your machine" section), because a bridge proves nothing about OS enforcement.
