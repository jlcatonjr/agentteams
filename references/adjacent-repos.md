...
**Last Updated:** 2026-08-15 by Repo Liaison (researchteam row audit — see Registry Revisions below; prior bulk event: 2026-08-10 durable-canonical-agent-format plan J.1 registration of 7 adjacent repos; prior fleet audit 2026-05-08)
...
| ~/githubrepositories/researchteam | .github/agents | secondary | yes | full_update | repo-liaison (scopes clearance for an agentteams-*initiated* push only — this repo instead pulls via its own CI, so the actual merge gate is the human operator; see 2026-08-15 audit note) | 2026-08-15 | Real, actively-updated consumer. Also has native `.goose/recipes/` and bridged `.claude/agents/`. Uses its own `researchteam` CLI wrapper (`.researchteam` marker) which auto-integrates on commit (pre-commit hook, layer1-only) and via `researchteam update` (layer1 + layer-2 project-specific file sync from `researchteam@main`). **2026-08-15 audit:** its weekly SHA-gated autosync CI (`agentteams-sync.yml`) had failed 5 consecutive runs (2026-07-13→2026-08-10) on a repo-setting block ("GitHub Actions is not permitted to create or approve pull requests") — real integration work was regenerated and force-pushed to `chore/agentteams-autosync` every week, then silently discarded because no PR ever opened. Recorded baseline (`.github/agentteams-autosync-ref`) was `c6a9cf6`, 54 commits behind agentteams `main` at audit time (corrected: the audit's first pass compared against a local feature-branch checkout, not `origin/main`, and overstated this as 62 — see plan.md §8). **Resolved same day:** permission fixed (`can_approve_pull_request_reviews=true`), CI re-dispatched twice (once to catch `main` advancing mid-session), landed [`researchteam#7`](https://github.com/jlcatonjr/researchteam/pull/7) fully current as of `61849fb`, awaiting human merge per the repo's own reviewer-gate policy. Full record: `tmp/by-week/2026-W33/researchteam-agentteams-integration-gap.plan.md`. |
...
| ~/githubrepositories/researchRepositories/OrthodoxLLM | .github/agents | secondary | yes | full_update | repo-liaison | 2026-08-15 | Real, actively-updated consumer; sibling of researchteam (same descriptor lineage — forked/customized brief.json, not a technical dependency). Also has native `.goose/recipes/` and bridged `.claude/agents/`. Uses the same `researchteam` CLI wrapper (`.researchteam` marker → `researchteam@main`). **2026-08-15:** had the identical autosync CI permission block as researchteam (found during that audit); fixed and merged same day ([`#2`](https://github.com/jlcatonjr/OrthodoxLLM/pull/2), current as of agentteams `61849fb`). Separately has a stale open PR #1 (targets an old `67655da`-era branch with its own uncommitted local changes) left untouched — superseded in substance but not closed by this session. |
| ~/githubrepositories/researchRepositories/SocialScienceHumanities | .github/agents | secondary | yes | full_update | repo-liaison | 2026-08-15 | Real, actively-updated consumer; same researchteam CLI-wrapper lineage as OrthodoxLLM, previously unregistered — found alongside it during the 2026-08-15 audit. Private repo holding sensitive research material with a documented prior incident (2026-08-07: a `researchteam update` overwrote `.gitignore` wholesale and briefly exposed 1041 corpus files before the local copy was upstreamed). **2026-08-15:** same CI permission block fixed and merged ([`#3`](https://github.com/jlcatonjr/SocialScienceHumanities/pull/3), current as of `61849fb`) — the resulting sync's `.gitignore` diff again dropped a project-specific exclusion (`Projects/PersonalityChange/`, sensitive personal content); verified before merging that `.git/info/exclude` already carried a durable independent copy (operator instruction, 2026-08-07) so nothing was exposed. Full account: `tmp/by-week/2026-W33/researchteam-agentteams-integration-gap.plan.md` §12. |
...
- **Total repos registered (enumerated below):** 21 (+1 2026-08-15: `SocialScienceHumanities`, previously a real, unregistered consumer)
- **Orchestrator present:** 8 (agentteams + 7 consumer repos registered 2026-08-10)
- **Update scope: full_update:** 19
...
| 2026-08-15 | Repo Liaison | Refreshed `researchteam` row: audit found its autosync CI had been failing weekly since 2026-07-13 (repo settings block on Actions-created PRs), leaving it 54 commits behind agentteams `main`. Resolved same day (permission fixed, PR jlcatonjr/researchteam#7 opened, fully current). Same-repo sibling `OrthodoxLLM` found to have the identical unresolved failure — flagged, not fixed (different, private repo, outside this task's scope) | Investigation triggered by an operator request to verify researchteam's integration status; see `tmp/by-week/2026-W33/researchteam-agentteams-integration-gap.plan.md` for full findings and remediation steps |
| 2026-08-15 | Repo Liaison | Operator authorized extending the fix to every local researchteam-pattern instance. Merged researchteam PR #7 + a docs follow-up (#8); fixed and merged the identical CI issue in `OrthodoxLLM` (#2) and previously-unregistered `SocialScienceHumanities` (#3, newly added as a row); caught a live Managed-File Overwrite Hazard on `SocialScienceHumanities` before merging (verified pre-existing local protection made it a non-issue); left `GeneralResearchTeam` untouched (no `.git` repository present) | Operator: "update each of the researchteam instances that are on this machine, and commit, push, and merge those updates"; full account in the same plan.md, section 12 |
