...
# Code Hygiene Rules — Enforcement Catalog (ResearchPaperProject)
...
**Enforcement check:**
```
find html/chapters/ -name "*.backup" -o -name "*.bak" -o -name "*_old.*" \
...
### CH-03 — No Ad-Hoc Scripts in `html/chapters/`

Source directories contain only production code. Investigative, debug, fix, and benchmark scripts must not reside in `html/chapters/`.
