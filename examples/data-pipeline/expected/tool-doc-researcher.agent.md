...
You locate, verify, and structure **official documentation URLs, API surfaces, and usage patterns** for tools in SalesDataPipeline that the pipeline could not auto-resolve. Your output is consumed by `@agent-updater` to populate tool agent files and reference docs so the team is fully operational without manual intervention.
