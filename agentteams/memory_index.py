...
def _rel_excluded(rel: str, entry: str) -> bool:
    """True when project-relative posix path *rel* is excluded by *entry*.

    Mirrors the ``memory_index_extra_dirs`` "directory-or-glob" convention: a plain
    entry excludes the path itself and everything under it (directory prefix); an
    entry with glob metacharacters is applied as an ``fnmatch`` pattern.
    """
    ...
    if rel == entry or rel.startswith(entry + "/"):
        return True
    return fnmatch.fnmatch(rel, entry)


def apply_memory_index_exclude(
    sources: Iterable[Path], project_root: Path, exclude: list
) -> list[Path]:
    """Drop *sources* matching any ``memory_index_exclude`` entry (F8 opt-out).

    Non-string or absolute entries are ignored. A source that resolves outside
    *project_root* (``os.path.relpath`` yields a ``..``-prefixed rel) is retained,
    since a project-relative entry cannot name it — no exception handling needed.

    Args:
        sources: Candidate index sources (Paths under *project_root*).
        project_root: The root the exclude entries are relative to.
        exclude: The manifest's ``memory_index_exclude`` list.

    Returns:
        *sources* with excluded paths removed, order preserved.
    """
    patterns = [e for e in exclude if isinstance(e, str) and e.strip() and not Path(e).is_absolute()]
    if not patterns:
        return list(sources)
    root = str(project_root)
    kept: list[Path] = []
    for p in sources:
        rel = Path(os.path.relpath(str(p), root)).as_posix()
        outside = rel == ".." or rel.startswith("../")
        if outside or not any(_rel_excluded(rel, e) for e in patterns):
            kept.append(p)
    return kept
