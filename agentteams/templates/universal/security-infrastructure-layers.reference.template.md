# Infrastructure Security Layers — {PROJECT_NAME}
...
- **Scope gate.** This reference applies only to projects that **build, deploy, run, or operate a
  program, server, or service**. If {PROJECT_NAME} ships no deployed software (e.g. a pure
...
- **Honest posture language:** record "engages as tested on {PROJECT_NAME}'s target," never
...
<!-- Project-owned: record {PROJECT_NAME}'s chosen layers, tools, deviations, and rationale below.
