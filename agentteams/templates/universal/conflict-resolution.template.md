---
name: Conflict Resolution — {PROJECT_NAME}
description: "Makes ACCEPT/REJECT/REVISE decisions on conflicts flagged by the conflict auditor in {PROJECT_NAME}"
...
# Conflict Resolution — {PROJECT_NAME}
...
For each conflict in `{CONFLICT_LOG_PATH}` with status `open`:
...
Update `{CONFLICT_LOG_PATH}`: change `status` to `resolved`, add `resolution` description.
