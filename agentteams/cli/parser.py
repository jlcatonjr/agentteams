...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--check-rank-strict",
        action="store_true",
        dest="check_rank_strict",
        help="AP-2 blocking variant of --check-rank: exits non-zero on any rank "
        "over-grant (opt-in CI gate; implies --check-rank).",
    )
    parser.add_argument(
        "--check-wiring",
        action="store_true",
        dest="check_wiring",
        help="P1-3: verify an emitted workspace-confinement sandbox block was actually "
        "merged into the live .claude/settings.json (it is inert until merged). Read-only, "
        "output-only; exits non-zero when a confinement was emitted but is not live.",
    )
    parser.add_argument(
        "--self",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help=(
            "CC-2: keep the emitted constitutional-gate hook fail-OPEN (a gate crash is a "
            "harness allow) even for a confined/exclusive privilege_profile, whose default "
            "is now fail-CLOSED (a crash emits a deny, so it cannot become a silent allow "
            "in a workspace that opted into a boundary). Use only if a buggy gate bricking "
            "the session is a worse risk than a missed block for this workspace."
        ),
    )
    parser.add_argument(
        "--capture-baseline",
