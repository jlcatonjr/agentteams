...
# Pipeline helpers carved to generate_helpers.py (CH-07); re-imported so call sites and
...
def _run_generate_inner(
    ...
    *,
    ...
    # silent allow in a workspace that opted into a boundary). Emitted onto the manifest only
    # when set, so a cooperative team (or the default) stays byte-identical.
    if getattr(args, "allow_fallback_fail_open", False):
        manifest["fallback_fail_open"] = True
    ...
    # -----------------------------------------------------------------------
    if args.check:
        return _handle_check(args, output_dir, manifest, adapter, project_name)
