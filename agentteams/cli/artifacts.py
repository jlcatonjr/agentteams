...
def resolve_host_features_and_advise(
    ...
    *, allow_unenforced: bool = True,
    ...
    manifest["host_features"] = merge_profile_features(
        explicit_tokens, manifest.get("privilege_profile"), framework_id
    )
...
def _advise_exclusive_inbound_hardening(manifest: dict, framework_id: str) -> None:
    ...
    NOT something agentteams enforces. So when `exclusive` is selected on a sandbox-capable
    target (Claude, or goose on macOS — ``is_sandbox_capable``), print and persist an
    advisory pointing at the operator OS filesystem hardening, mirroring the honest
    `privilege_profile_advisory` discipline (never a silent claim of protection). No-op for
    other profiles/hosts (unenforced-host is already warned by resolve_host_features_and_advise).
    ...
    if manifest.get("privilege_profile") != "exclusive" or not is_sandbox_capable(framework_id):
        return
    message = (
...
def _memory_index_sources(manifest: dict, output_dir: Path) -> list[Path]:
    ...
    Note what this *automatic* rule is *not*: it is not "exclude gitignored paths".
    ...
    therefore about *scratch*, identified by directory name.

    Separately, an operator may *opt in* to dropping specific durable sources via
    the manifest's ``memory_index_exclude`` (project-relative paths/globs). That is
    a deliberate, declared choice — e.g. when durable content has been relocated to
    a private repository and must not be embedded in the committed, public index —
    and is distinct from the automatic scratch rule above: it removes exactly what
    the operator names, nothing by inference. Prefers the manifest's ``existing_project_path`` (the
        ...
        for raw in extra:
            ...
            for c in candidates:
                ...
                try:
                    ...
                    continue
                sources.append(c)
    exclude = manifest.get("memory_index_exclude")
    if isinstance(exclude, list) and exclude:
        from agentteams.memory_index import apply_memory_index_exclude
        sources = apply_memory_index_exclude(sources, project_root, exclude)
    return sources
