...
# HTML-comment region (``<!-- AGENTTEAMS-BRIDGE:BEGIN … -->``, mirrors fences._BRIDGE_FENCE_BEGIN_RE)
# and a real subagent stub carries ``source_sha256:`` as a line-anchored front-matter key (see
...
def _claude_kind(ws: Path) -> str:
    ...
    for entry in (ws / "CLAUDE.md", *(claude.rglob("*.md"))):
        try:
            ...
            continue
        # Require the structural HTML-comment fence, not a bare substring mention (see above).
        if _BRIDGE_FENCE_RE.search(text):
            return "bridge"
    agents_dir = claude / "agents"
    if agents_dir.is_dir():
        for f in agents_dir.glob("*.md"):
            try:
                # Require source_sha256 as a line-anchored front-matter key, not prose (see above).
                if _SUBAGENT_STUB_RE.search(f.read_text(encoding="utf-8", errors="ignore")):
                    return "bridge"
            except OSError:
                continue
