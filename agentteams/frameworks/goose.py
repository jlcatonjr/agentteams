...
def _goose_capabilities_content(project_name: str) -> str:
    ...
    return (
        ...
        "wiring needed:\n\n"
        "If this project has its own optional `agentteams.research` module installed\n"
        "(`pip install agentteams[research]`) — check via `python -m agentteams.research --help`\n"
        "before assuming otherwise, the same verify-first discipline as any other CLI tool — it\n"
        "reaches search/fetch this way. Separately: none of the\n"
        "builtin extensions execute JavaScript or drive a real browser — `computercontroller`'s\n"
        "`web_scrape` is a plain fetch, not a renderer. A page that needs JavaScript to show its\n"
        "real content is a job for `python -m agentteams.research browser \"<url>\"` (the same\n"
        "module's separate `[browser]` extra, if installed), not an extension. See\n"
