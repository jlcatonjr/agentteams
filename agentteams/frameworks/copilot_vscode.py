...
from .format_spec import AGENT_FILE_EXTENSION, FORMAT_SPECS
from agentteams.yaml_frontmatter import parse_yaml_front_matter as _parse_yaml_front_matter

#: The VS Code Copilot agent-file front-matter contract, now DERIVED from the single source
#: (format_spec) rather than duplicated — both ``required_front_matter_keys()`` and
#: ``_REQUIRED_YAML_KEYS`` (the emit-time presence check) read this, so nothing can drift
#: from the spec (audit 2026-W39 hygiene #1). Guarded by tests/test_format_spec_single_source.py.
_VSCODE_FRONT_MATTER_KEYS = FORMAT_SPECS["copilot_vscode"].emitted_front_matter_keys
