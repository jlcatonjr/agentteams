...
def _goosehints_content(project_name: str) -> str:
    ...
    G5 FIX (2026-08-15, references/goose-agent-infrastructure-expert.md): the
    leading `@AGENTS.md` import line is removed — current Goose docs confirm
    `CONTEXT_FILE_NAMES` defaults to `["AGENTS.md", ".goosehints"]`, i.e. Goose
    reads both files natively without an explicit import; the line was
    redundant, plausibly double-loading AGENTS.md into every session. The
    bridge-entry `.goosehints` (agentteams/bridge.py, a separate code path)
    carried the identical redundancy and is fixed in the same pass.
    """
    return (
