...
- **A capability grant can now be anchored to the issuer's tree, and the anchor is signed.** A
  grant spec may declare `issuer_root`; when present it is recorded and bound into the grant
  signature, and an **absolute** `target_path` that resolves outside it is rejected — both at
  issue time and on the generation-time sandbox-widening path. This closes the P2-4 gap the shape
  guard alone left (it permitted any well-shaped absolute path); because `issuer_root` is signed,
  an attacker cannot forge a wider anchor (e.g. `/`) to defeat the check. **Fully backward
  compatible:** `issuer_root` is an *optional appended signed field* — a grant issued without it
  reproduces the exact pre-G-6 signing payload, so existing signed grants keep verifying and get
  shape-guard-only treatment. `agentteams/cli/grants.py` (`_signed_values`,
  `_assert_target_within_issuer_root`, `issue_grant(issuer_root=…)`), `cli/commands.py`
  (spec `issuer_root`). 7 new tests (containment, tamper-breaks-signature, backward-compat,
  widening-path); `grants.py` re-manifested. `@security` CONDITIONAL PASS.

### verified (Cluster B — OS-sandbox enforcement, product arm, end-to-end)

- **The emitted `sandbox` block is now verified to enforce end-to-end through real Claude Code
  (macOS, `claude -p`, v2.1.246), not just at the raw-Seatbelt mechanism level.** Escape tests
  with paired negative controls confirm: a write to `$HOME` (outside `allowWrite` + temp) is
  **denied**; the D-3 `denyWrite` blocks an in-sandbox agent from flipping the
  `enforce_decision_signing` switch (`references/agent-privilege.json`); a `denyRead` secret
  cannot be read; and legitimate in-root writes/reads still succeed. Encoded as the opt-in
  regression guard `tests/test_os_sandbox_product_enforcement.py` (`RUN_CLAUDE_SANDBOX_ITEST=1`).
  Docs upgraded (`api-reference/workspace-privilege-scoping.md`). Test-design caveat recorded: the
  sandbox allows the working dir **and the session temp dir**, so an escape target must be
  non-temp (a `/tmp` target is allowed and proves nothing).
- **P3-3 resolved:** the `~`-relative `denyRead` form (agentteams' default) is verified to enforce
  end-to-end — Claude Code expands `~` before the Seatbelt deny (a tilde-denied secret is blocked
  while a different, non-denied HOME file stays readable). It is *not* a silent no-op at the
  product level; the `expanduser` opt-in stays as defense-in-depth, not a correctness requirement.
  Guarded by `test_os_sandbox_product_enforcement.test_p3_3_tilde_denyread_expands_and_enforces`.
  Remaining open: the Linux/bwrap product arm (testLinux VM).

### hardened (Cluster D — tamper-resistance substrate: integrity-pin the enforcers)

- **The privilege emitters and the constitutional-gate hook are now integrity-tracked.**
  Previously `ENFORCEMENT_MODULES` covered the gate/decision-log/scanner/fences but **not** the
  code that *emits* the controls, nor the gate hook itself — so a silent edit to grant
  issuance, the rank-conformance ceiling, or the hook would weaken the boundary without
  tripping probe E4 (the "gate tracked, emitters not" gap; the shared unenforced substrate both
  security threads rest on). Added to `ENFORCEMENT_MODULES`: `cli/grants.py` (P2 grant
  issue/verify + hash-chain) and `rank_conformance.py` (AP-2 `tools:`-vs-rank ceiling) — **D-2**;
  and the three constitutional-gate hook files (`templates/universal/hooks/`, `.claude/hooks/`,
  `.github/hooks/constitutional-gate.py`) — **D-1**. A silent flip of `_FAIL_CLOSED_ON_ERROR` or
  a gutted `_decide` now trips E4. Tightening, not widening (extends SHA-256 coverage only; no
  `tools:`/sandbox/grant change). Membership + byte-mutation regression tests in
  `tests/test_verify_integrity_enforcement.py`. Two `@security` CONDITIONAL PASS rows recorded.
  `frameworks/claude.py` deferred to a `@code-hygiene`-approved extract-to-`_sandbox_emit.py`
  refactor (D-2b) to avoid regen friction on the high-churn adapter.
- **The sandbox emitter is now its own integrity-tracked module, and the
  `enforce_decision_signing` switch is write-protected inside the sandbox.** **D-2b:** the
  sandbox/`denyRead` emission moved out of the 775-line `frameworks/claude.py` (now 552) into a
  small stable `frameworks/_sandbox_emit.py`, which is added to `ENFORCEMENT_MODULES` (claude.py
  re-exports the names; behavior unchanged; 56 privilege tests green). **D-3:** every
  `confined`/`exclusive` sandbox block now emits `filesystem.denyWrite` for the control plane —
  `references/agent-privilege.json` (the switch, which lives inside the write root and is *not*
  under the auto-protected `.claude/`) and `.claude/hooks/constitutional-gate.py`. `denyWrite`
  takes precedence over `allowWrite`, and the switch/hook are only ever written by the operator
  at generate/update time (outside any sandbox), so denying the in-sandbox agent costs no
  legitimate workflow. Docs: `api-reference/workspace-privilege-scoping.md`.

### added (CC-2 — profile-dependent fail-closed constitutional gate)

- **A `confined`/`exclusive` `privilege_profile` now emits the constitutional-gate hook
  fail-CLOSED.** By default the PreToolUse gate fails **open** on an internal crash (the
  harness treats a non-zero exit as an allow) — accepted for a cooperative workspace, where
  a hook that can brick a session gets deleted. But a workspace that opted into a boundary
  must not have it silently dropped by a gate error, so for confined/exclusive agentteams
  emits the hook with `_FAIL_CLOSED_ON_ERROR = True`: an unexpected crash instead emits a
  `deny`. The handler is a process-boundary reporting handler (acts + reports, not a
  swallow). **`--allow-fallback-fail-open`** restores the fail-open default for those
  profiles (operator escape hatch). Cooperative teams are unchanged and byte-identical.
  Emitted via `frameworks/claude.py::_apply_fail_closed_policy`; opt-out carried as
  `fallback_fail_open` on the manifest (emitted only when set). Tests in
  `tests/test_workspace_privilege_scoping.py` (emit injection + hook runtime).

### added (privilege CLI surface — `--check-wiring`, W19 advisory)

- **`--check-wiring`** exposes the P1-3 `verify_sandbox_wiring` check on the CLI: confirms an
  emitted `sandbox` block was merged into the live `.claude/settings.json` (exits non-zero
  when a confinement was emitted but is not live/enabled, an escape hatch is open, or write
  roots differ). Read-only, output-only.
- **`--check-rank` now also prints a W19 advisory** naming any agent that declares no
  capability key (an implicit maximal grant), without affecting its exit code.
- `agentteams/cli/parser.py` is added to the CH-07 `LENGTH_ALLOWLIST` (a monolithic argparse
  definition that grows per-flag and is not sensibly split; operator decision 2026-08-24) so
  these flags and `--allow-fallback-fail-open` can land.

### hardened (privilege follow-ups — grant widening guard, wiring check, W19 detector)

- **P2-4: the grant target-path guard now runs on the real enforcement path.**
  `granted_write_roots` (the generation-time widening that merges a grant's `target_path`
  into a holder's sandbox `allowWrite`) re-asserts `_reject_unsafe_target_path` on each
  `write` grant, failing closed — so an unsafe value can never reach `allowWrite` by a path
  that skipped the issue-time check. Defense in depth (`target_path` is signature-covered,
  so no behavior change for legitimate grants). `@security` cleared it PASS (narrowing).
  Full `path_within(issuer_root)` containment remains a tracked follow-up (needs
  `issuer_root` recorded in the signed grant). Tests in `tests/test_capability_grants.py`.
- **P1-3: `verify_sandbox_wiring` (frameworks/claude.py)** — a read-only, output-only check
  that confirms an emitted `sandbox` confinement block was actually merged into the live
  `.claude/settings.json` (it is inert until merged), catching the "unmerged boundary — NOT
  enforced" silent failure, an open escape hatch (`allowUnsandboxedCommands`), or mismatched
  write roots. It never echoes values read from the operator's settings. Tests in
  `tests/test_workspace_privilege_scoping.py`. (A `--check-wiring` CLI entry point is a
  pending follow-up — `cli/parser.py` is at the CH-07 line ceiling, so exposing it needs a
  `@code-hygiene` call on the allowlist-vs-split question.)
- **W19: `rank_conformance.agents_missing_capability_key`** — the complement of the rank
  validator (which *skips* an agent with no `tools:` key): it *flags* those agents, since a
  missing capability key is an implicit maximal grant the validator is structurally blind
  to. Reporting-only — it names the agents, it does not invent a `tools:` value (that is an
  operator/`@security` decision). Tests in `tests/test_rank_conformance.py`.

### documented (privilege honesty — foundations, bridge non-goal)

- **Design-foundations note** added to `workspace-privilege-scoping.md` citing only the
  *verified* principles (least-privilege, fail-safe-defaults, complete-mediation,
  confused-deputy avoidance, keyed HMAC integrity) as rationale — explicitly **not** a claim
  that the design formally realizes a reference monitor or macaroons (RH-1).
- **Bridge non-goal declared** in `references/bridge-refresh-safety.md`: a
  `--bridge-refresh`/`--bridge-merge` does **not** propagate a target's privilege scoping
  (sandbox boundary, P2 grants, inbound advisory) into a foreign repo — these are
  host/operator-specific trust state; a bridged target that needs confinement is generated
  natively on its own host and re-verified there (CC-8).

### added (workspace privilege scoping — P3-3 opt-in absolute denyRead)

- **`resolve_deny_read_abspath` (project-description field, default `false`)** closes the
  P3-3 silent-no-op risk in the `exclusive` profile's read-exclusion. The emitted
  `denyRead` list is `~/`-relative and relies on Claude Code expanding `~`→`$HOME` before
  the OS deny — an assumption agentteams cannot verify from its side; if it is false, every
  default deny entry silently no-ops while the config still looks protective. When the new
  flag is set, each `~/` entry is resolved to an `expanduser`'d **absolute** path at emit
  time, removing the dependency. The default (`false`) is unchanged and **byte-identical**
  (portable `~/` form + its `~`-expansion self-check comment); the opt-in swaps in a
  host-specific-portability comment. Additive on both schemas; emitted only when true.
  Tests in `tests/test_workspace_privilege_scoping.py` (`test_resolve_deny_read_abspath_*`).
  The opt-in removes the `~` dependency; it does **not** by itself prove enforcement — that
  still owes the in-sandbox escape test (RH-4). Exposed as a description field only (no CLI
  flag this pass).

### added (agent-position privilege — opt-in blocking rank check, SP-04)

- **`--check-rank-strict`** — the blocking disposition of `--check-rank` (AP-2). Same
  read-only rank-conformance audit, but **exits non-zero** when any agent's declared
  `tools:` exceeds its taxonomy-rank ceiling, instead of warn-only. Implies `--check-rank`.
  The intended CI gate for the agent-position axis, to switch on once `PER_AGENT_OVERRIDES`
  is ratified; the default `--check-rank` stays warn-only (exit 0) so nothing changes until
  a project opts in. Finding severity is unchanged — only the exit code differs (disposition
  lives in a new thin `rank_conformance.disposition_exit_code`). The shipped Claude team
  validates clean under strict. Man page + `cli-reference.md` updated; tests in
  `tests/test_rank_conformance.py`.
...
### fixed (source remediation from the exampleSecurityRepo handoff)
...
- **Backlog 33 → 28 open, 1 wontfix.** `--refresh-index` was already the narrow reindex path #20
  ...
  matching the *correction text*. And `ExampleResearchGroup` is a parent folder holding research
...
- `example-research` and `ExampleResearchGroup` both classify as `project_type: research`, both produce
...
- **Discovered doing so:** agent-file YAML front matter lies *outside* every `AGENTTEAMS` fence,
  ...
  `references/agentteams-remediation-log.csv`, along with the finding that `ExampleResearchGroup`
...
- **`agentteams.security_refs.build_security_placeholders()` no longer degrades a real cached CVE
  ...
  across subsequent runs. Root-caused and reported from `example-org/example-collector`
...
- **Automatic past-day work-summary backfill + trigger.** Promotes the
  example-collector-developed feature into the framework: every generated team
...
- **Per-strategy memory-index thresholds (v=2).** The four
  ...
  Validated against `example-collector`: corpus 69 → 198 docs,
...
### --migrate hardening + example-research test update (2026-05-22)

Surfaced by using the `example-research` repo (12 fenced / 17 legacy agent files) as
...
- **Finding (not yet a fix):** `--migrate` is `--overwrite`-based, so it
  ...
  against `example-research` (a curated literature list in `primary-producer`), then
...
- **History rewrite: ExampleOrg references removed from commit history** — all commit messages and tracked file content matching `example-org`, `/example-org/`, and `vk-[a-z0-9-]+` patterns were replaced with `REDACTED_REPO` / `REDACTED_SERVICE` using `git filter-repo`. Pre-rewrite HEAD: `10b8bfc`. Post-rewrite HEAD: `b67c514`. Mirror backup retained at `tmp/by-week/2026-W19/rewrite-backups/agentteams.mirror.20260504-160919.git`. Post-rewrite verification: MSG_HITS=0, TRACKED_HITS=0.
- **Commentary scope rule** — added constitutional rule to `.github/copilot-instructions.md`, `orchestrator.agent.md`, `agent-updater.agent.md`, and `work-summarizer.agent.md`: ExampleOrg repository operational updates must not appear in AgentTeams commit/PR notes, comments, or work summary narrative unless the entry documents a direct, material change to files inside this repository.
