...
- **Typed handoffs: dated promotion of `PAYLOAD_UNTYPED` to HARD on 2026-07-01** — `agentteams/handoff_payloads.py` defines `PAYLOAD_UNTYPED_HARD_DATE = date(2026, 7, 1)`. Before that date, plan `.steps.csv` rows missing `payload_schema_out` / `payload_schema_in` produce a `PAYLOAD_UNTYPED` finding at WARN severity; on and after that date the same finding is emitted at HARD severity. Severity is enforced mechanically — no editorial path softens it. Operators should type all `.steps.csv` handoffs before the cutoff or accept HARD findings.
- **Conflict-auditor codes for handoff payloads** — `agentteams/templates/universal/conflict-auditor.template.md` adds a fenced `handoff_payload_codes v=1` section declaring two new codes: `PAYLOAD_UNTYPED` (WARN until 2026-07-01, HARD thereafter) and `PAYLOAD_MISMATCH` (HARD). When auditing `.steps.csv` artifacts, `@conflict-auditor` invokes `agentteams.handoff_payloads.audit_handoff_chain` and surfaces the findings.
...
- **Typed handoff payload substrate (`agentteams.handoff_payloads`)** — new module providing typed handoffs between agents along plan `.steps.csv` rows. Public API: `load_payload_schema` (path-restricted loader rejecting URLs, absolute paths, `..` segments, and non-matching globs), `strip_llm_visible_text` (mechanical sanitizer removing `description`/`title`/`$comment`/`examples` from schema text before any LLM sees it), `validate` (multiprocessing-isolated validator with cross-platform wallclock timeout and depth/cycle pre-walk), `audit_handoff_chain` (pure-Python `$id`-only comparator for adjacent handoff steps), and `Finding` / `PayloadSchemaError` / `SchemaInvalid` types. Constants: `PAYLOAD_UNTYPED_HARD_DATE = 2026-07-01`, `MAX_DEPTH = 32`, `VALIDATE_TIMEOUT_SECONDS = 2.0`. Mitigates six vulnerabilities (V1 path traversal, V2 schema poisoning, V3 LLM injection via schema text, V4 version drift, V5 DoS via recursive/slow schemas, V6 opt-in bypass).
- **Plan steps reader (`agentteams.plan_steps`)** — new module exposing `read_steps(path)`, a tolerant CSV reader for `tmp/by-week/**/*.steps.csv` plan artifacts. Handles quoted multi-line cells (e.g. multi-line `notes` columns).
- **Meta-schema for handoff payloads** — `schemas/handoff-payload-meta.schema.json` enforces the `<slug>/v<n>.schema.json` `$id` pattern, `additionalProperties: false`, and a non-empty `required` array on every concrete payload schema.
- **Worked-example payload schema** — `schemas/handoff-payloads/conflict-audit-result.v1.schema.json` documents the `@conflict-auditor` finding payload shape.
- **Runtime dependency `jsonschema>=4.20`** — added to `pyproject.toml` for handoff payload validation.
- **Tests: 9 new tests in `tests/test_handoff_payloads.py`** — cover V1-V6 mitigations (path traversal, meta-schema rejection, LLM-text stripping, `$id` mismatch detection, depth/timeout bounds, dated WARN→HARD promotion) plus the multi-line CSV reader.
