# Code Hygiene Rules — Enforcement Catalog ({PROJECT_NAME})
...
**Enforcement check:**
```
find {PRIMARY_OUTPUT_DIR} -name "*.backup" -o -name "*.bak" -o -name "*_old.*" \
...
### CH-03 — No Ad-Hoc Scripts in `{PRIMARY_OUTPUT_DIR}`

Source directories contain only production code. Investigative, debug, fix, and benchmark scripts must not reside in `{PRIMARY_OUTPUT_DIR}`.
