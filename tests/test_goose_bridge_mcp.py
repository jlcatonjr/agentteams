...
def _server(**kw):
    base = {
        ...
        "server_id": "example-pg", "domain": "data", "trust_tier": "first-party",
...
def test_token_plus_artifact_wires_orchestrator_server(tmp_path):
    ...
    assert 'name: "example_pg"' in recipe
...
def test_merge_regenerates_recipe_so_new_servers_propagate(tmp_path):
    ...
    assert 'name: "example_pg"' in (tmp_path / _RECIPE_REL).read_text()   # regenerated + wired
