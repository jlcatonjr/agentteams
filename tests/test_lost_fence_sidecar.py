...
def test_merge_result_carries_lost_body_when_shrink_fires():
    ...
    assert "example-collector" in saved
    assert "example-research" in saved
...
def test_emit_all_writes_lost_fence_sidecar(tmp_path):
    ...
    existing_body = (
        "- rule 1 cites `example-collector`\n"
        "- rule 2 cites `example-research`\n"
        "- rule 3 cites `example-api-utils`\n"
        "- rule 4 cites `example_data_collection`\n"
    )
