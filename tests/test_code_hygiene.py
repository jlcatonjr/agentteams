...
LENGTH_ALLOWLIST: frozenset[str] = frozenset({
    ...
    #
    # cli/parser.py: the single argparse definition for the whole CLI surface — one flat
    # add_argument sequence that grows by ~7 lines per new flag and is not sensibly split
    # (an arg parser wants to be read top-to-bottom in one place). Allowlisted (operator
    # decision 2026-08-24, CC-2/CLI-exposure) so privilege CLI flags (--check-wiring, and
...
BROAD_EXCEPT_BASELINE = 18      # 16→18: the SAME fail-closed process-boundary handler,
                                ...
                                # entry below — a boundary catch that emits a deny and reports,
                                # not a swallow — so an installed gate crash fails closed too.
                                # 15→16: CC-2 fail-closed handler in the emitted
                                # constitutional-gate hook (_entrypoint). The broad catch is a
                                # process-BOUNDARY handler that ACTS (emits a deny) and reports —
                                # not a swallow — so a gate crash under a confined/exclusive
                                # profile fails closed instead of a silent harness allow.
                                # 14→15: tests/conftest.py's tracked-artifact detector. The
