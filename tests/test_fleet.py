...
def test_claude_kind_bridge_via_real_html_fence(tmp_path):
    # A genuine bridge fence is an HTML comment, not a bare marker mention.
    ws = tmp_path / "ws"
    _mk_agent(
        ws / ".claude" / "agents",
        "x.md",
        "<!-- AGENTTEAMS-BRIDGE:BEGIN orchestrator v=1 -->\nbody\n",
    )
    assert fleet._claude_kind(ws) == "bridge"


def test_claude_kind_not_bridge_on_prose_mentioning_fence(tmp_path):
    # Regression (fleet.py false-positive, 2026-08-25): a native team whose agent docs *quote*
    # the fence marker string in prose must NOT be misclassified as a bridge (exampleRecoveryRepo repro).
    ws = tmp_path / "ws"
    _mk_agent(
        ws / ".claude" / "agents",
        "x.md",
        "This native team documents the AGENTTEAMS-BRIDGE:BEGIN fence syntax.\n",
    )
    (ws / ".github" / "agents").mkdir(parents=True)
    (ws / ".github" / "agents" / "_build-description.json").write_text("{}", encoding="utf-8")
    assert fleet._claude_kind(ws) == "direct"
...
def test_claude_kind_not_bridge_on_prose_mentioning_source_sha256(tmp_path):
    # Regression: prose naming the source_sha256 provenance field is not a subagent stub.
    ws = tmp_path / "ws"
    _mk_agent(
        ws / ".claude" / "agents",
        "x.md",
        "Bridge stubs carry a source_sha256 provenance header for drift detection.\n",
    )
    assert fleet._claude_kind(ws) == "ambiguous"
