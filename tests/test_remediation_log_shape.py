...
# The remediation log is an operational record: gitignored and absent from the
# public-release checkout (and from CI). Skip the whole module when it is not
# present, mirroring tests/test_conflict_log_shape.py. `_rows()` also degrades to
# [] so the module-level parametrize below does not error at collection time.
pytestmark = pytest.mark.skipif(
    not LOG_PATH.exists(),
    reason="agentteams-remediation-log.csv is gitignored and absent from this checkout",
)
