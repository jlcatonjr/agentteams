...
SCHEMA_PATH = REPO_ROOT / "agentteams" / "schemas" / "memory-index.schema.json"
...
def test_memory_index_exclude_drops_named_glob_and_dir(tmp_path):
    """memory_index_exclude removes matching sources (dir-prefix and glob), and
    leaves non-matching siblings indexed. Guards the re-leak fix: relocated durable
    content must not be embedded in the committed index."""
    root = tmp_path / "project"
    (root / "workSummaries" / "daily").mkdir(parents=True)
    (root / "workSummaries" / "daily" / "2026-04-28.md").write_text("moved summary")
    (root / "references").mkdir()
    (root / "references" / "goose-x-2026-06-22.adversarial.md").write_text("moved audit")
    (root / "references" / "openweights-security-model-eval.report.md").write_text("moved report")
    (root / "references" / "policy.md").write_text("keep policy")  # not excluded
    (root / "references" / "goose-agent-infrastructure-expert.md").write_text("keep expert")  # no -2026-
    output_dir = root / ".github" / "agents"
    output_dir.mkdir(parents=True)

    manifest = {
        "existing_project_path": str(root),
        "memory_index_exclude": [
            "workSummaries",
            "references/*-2026-*.adversarial.md",
            "references/openweights-security-model-eval.report.md",
        ],
    }
    source_set = {str(p) for p in build_team._memory_index_sources(manifest, output_dir)}
    # excluded
    assert str(root / "workSummaries" / "daily" / "2026-04-28.md") not in source_set
    assert str(root / "references" / "goose-x-2026-06-22.adversarial.md") not in source_set
    assert str(root / "references" / "openweights-security-model-eval.report.md") not in source_set
    # kept (siblings that must stay public/indexed)
    assert str(root / "references" / "policy.md") in source_set
    assert str(root / "references" / "goose-agent-infrastructure-expert.md") in source_set


def test_memory_index_exclude_absent_or_empty_is_todays_behavior(tmp_path):
    """No memory_index_exclude key (or empty list) leaves indexing unchanged."""
    root = tmp_path / "project"
    (root / "workSummaries").mkdir(parents=True)
    (root / "workSummaries" / "daily.md").write_text("summary")
    output_dir = root / ".github" / "agents"
    output_dir.mkdir(parents=True)
    base = {"existing_project_path": str(root)}
    without = {str(p) for p in build_team._memory_index_sources(base, output_dir)}
    empty = {str(p) for p in build_team._memory_index_sources({**base, "memory_index_exclude": []}, output_dir)}
    assert without == empty
    assert str(root / "workSummaries" / "daily.md") in without


def test_memory_index_exclude_ignores_absolute_entries(tmp_path):
    """Absolute exclude entries are ignored (cannot name a project-relative source)."""
    root = tmp_path / "project"
    (root / "references").mkdir(parents=True)
    (root / "references" / "policy.md").write_text("keep")
    output_dir = root / ".github" / "agents"
    output_dir.mkdir(parents=True)
    manifest = {
        "existing_project_path": str(root),
        "memory_index_exclude": ["/etc", str(root / "references")],
    }
    source_set = {str(p) for p in build_team._memory_index_sources(manifest, output_dir)}
    assert str(root / "references" / "policy.md") in source_set
