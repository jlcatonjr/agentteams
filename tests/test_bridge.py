...
def test_bridge_manifest_source_dir_is_relative_when_nested_in_output_root(tmp_path: Path):
    ...
    self-hosting repos like agentteams/example-research/ExampleDomainLLM are actually bridged),
...
def test_goose_bridge_entry_advertises_research_capability(tmp_path: Path):
    ...
    # G5 fix (2026-08-15): .goosehints no longer imports @AGENTS.md — Goose
    # reads AGENTS.md natively, so the reachability guarantee now rests on
    # that native discovery, not an explicit import line.
    hints = (out_root / ".goosehints").read_text(encoding="utf-8")
    assert "@AGENTS.md" not in hints
...
def test_bridge_goose_first_time_creates_exact_file_set(tmp_path: Path, source_framework: str):
    ...
    # G5 fix (2026-08-15): no @AGENTS.md import — Goose reads AGENTS.md natively.
    assert "@AGENTS.md" not in (out_root / ".goosehints").read_text(encoding="utf-8")
...
def test_bridge_goose_merge_skips_unfenced_goosehints(tmp_path: Path):
    ...
    generated = "Goose operational notes (generated by agentteams)\n"
...
def test_canonical_source_collect_and_inventory(tmp_path: Path):
    ...
    # Canonical front matter has no user-invocable key — honest "no", not a guess.
