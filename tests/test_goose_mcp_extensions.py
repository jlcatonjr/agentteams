...
def _server(**kw):
    base = {
        ...
        "server_id": "example-postgres", "domain": "data-access",
        "trust_tier": "first-party", "transport": "stdio",
        "command": "uvx", "args": ["mcp-server-postgres"],
        "auth": {"mechanism": "env", "credential_ref": "EXAMPLE_PG_DSN"},
...
def test_default_off_is_byte_identical_to_baseline():
    ...
    no_mcp_at_all = GooseAdapter().render_agent_file(
        ...
        {"project_name": "Example", "output_files": [{"path": ".goose/recipes/data-analyst-expert.agent.md"}]},
    )
...
def test_stdio_server_wired():
    ...
    assert 'name: "example_postgres"' in recipe          # identifier-normalized
    assert 'cmd: "uvx"' in recipe
    assert '- "mcp-server-postgres"' in recipe
    assert 'env_keys:' in recipe and '- "EXAMPLE_PG_DSN"' in recipe
...
def test_non_read_only_servers_skipped_and_surfaced(kw, reason):
    ...
    assert "# agentteams MCP: example-postgres not wired" in recipe
