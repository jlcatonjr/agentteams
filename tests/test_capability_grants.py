...
@pytest.mark.parametrize("bad", ["/", "~", "~/secrets", "../escape"])
def test_granted_write_roots_rejects_unsafe_target_on_widening_path(tmp_path, monkeypatch, bad):
    # P2-4 defense in depth: a `write` grant with an unsafe target that somehow reaches the
    # generation-time widening path (bypassing the issue-time guard) must fail closed there,
...
# --------------------------------------------------------------------------
# G-6 (P2-4 full containment): a signed issuer_root anchors an absolute target
# --------------------------------------------------------------------------

def test_backward_compat_grant_without_issuer_root_still_verifies(tmp_path):
...
def test_issue_rejects_absolute_target_outside_issuer_root(tmp_path):
    ...
    issuer = (tmp_path / "issuer").resolve()
    issuer.mkdir()
    outside = (tmp_path / "elsewhere" / "evil").resolve()  # well-shaped, absolute, but outside
    with pytest.raises(grants.GrantError, match="outside its signed issuer_root"):
        _issue(tmp_path, target_path=str(outside), issuer_root=str(issuer))


def test_issue_rejects_non_absolute_issuer_root(tmp_path):
    _write_roster(tmp_path, "alice")
    with pytest.raises(grants.GrantError, match="must be an absolute path"):
        _issue(tmp_path, target_path="/abs/b/shared", issuer_root="relative/issuer")


def test_tampering_issuer_root_breaks_signature(tmp_path):
    _write_roster(tmp_path, "alice")
    issuer = (tmp_path / "issuer").resolve()
    (issuer / "shared").mkdir(parents=True)
    rec = _issue(tmp_path, target_path=str(issuer / "shared"), issuer_root=str(issuer))
...
def test_granted_write_roots_enforces_issuer_root_containment(tmp_path, monkeypatch):
    """On the widening path, a (synthetic) write grant whose absolute target escapes its
    signed issuer_root fails closed — the target can never become real OS write reach."""
    issuer = str((tmp_path / "issuer").resolve())
